...
6. 设计灯具的布置方式（如：按行列均匀排列），必须是**矩形均匀排布**，不可以排布为三角形，多边形等，并输出每盏灯的中心坐标和安装高度（单位：mm）；
     - 输出灯具同一的的安装角度 `fixture_rotations_degrees`，单位为度（°）；
     - 默认情况下，灯具横向放置（与房间宽度平行），角度为 `0°`；
     - 若用户在自定义指令中明确要求“纵向布置”或“沿高度方向排布”，则角度为 `90°`；
...
10. 设计插座之间的连线方式：**左右两侧墙体上的插座必须连接起来，连接线路必须沿着房间墙壁边缘布置，线路终点为最近的房间角点**，连接顺序应整齐、连续，符合实际施工路径，输出每段插座之间的连接线段坐标，格式为起点终点坐标对；
11. 设计开关位置：应靠近门的位置布置开关同时要和最近的灯具的位置**对齐**，有且仅有一个开关；
12. 为全部灯具信息生成一个用于注释说明的插入坐标 annotation_position_mm，坐标要位于房间内部不能在边界上，不得与灯具、插座、开关的位置重合；
13. 最后以 JSON 格式输出灯具、插座、开关的布置信息，供程序解析使用：
...
{{
  ""lighting_design"": {{
    ...
    ""fixture_rotations_degrees"": 0 ,
    ""annotation_position_mm"": [x, y] , 
    ""fixture_positions_mm"": [
      [x1, y1],
      [x2, y2],
      ...
  ...
  ""power_outlet_connection_line_mm"": [
      [[x_near_door, y_near_door], [x_socket, y_socket]]
    ]
  }},
  ""socket_positions"": [
    {{
